...
meanspeed <- cats_uk %>% 
  group_by(tag_id) %>% 
  summarise(avg_speed = mean(ground_speed,na.rm = T))

cattos <- left_join(cats_uk_reference,meanspeed)

cattos <-cattos %>% 
  select(!c(animal_taxon,study_site,animal_id))

cattos <- cattos %>%
  mutate(deploy_length = deploy_off_date-deploy_on_date) %>% 
  select(!c(deploy_on_date,deploy_off_date))
